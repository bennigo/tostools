...
Every `tos` verb is reachable from `tosGPS`. Nine are shared: **eight plain
aliases** (`audit`, `fleet`, `station`, `device`, `location`, `visit`,
`contact`, `owners`) delegating unchanged via the `_PLAIN_ALIASES` table in
`tosGPS.py`, plus **`search`, the one profiled verb** — GPS-profiled under
`tosGPS` (subtype pinned, non-GPS attributes refused), general fleet query
under `tos`.

A test asserts `set(_PLAIN_ALIASES) | {"search"} == tos.KNOWN_SUBCOMMANDS`, so
a new verb cannot be silently forgotten. If it fails after you add a verb,
decide alias-vs-profile — don't widen the assertion.

Full detail: **[`docs/cli-surface.md`](docs/cli-surface.md)**.
...
Three traps worth knowing before trusting a result: `~` globs are **fully
anchored** (`HVE*` is starts-with, not contains); device-selector matching is
**existential** across a station's devices; and `!=` is **satisfied by an
absent value**.

Full reference: **[`docs/search-selectors.md`](docs/search-selectors.md)**.

### `tos audit version-chains`

Compares the Septentrio `firmware_version` / `software_version` chains, which
carry one physical quantity on two attribute codes. **Period-aware** — an
open-period-only comparison misses a chain whose history diverged while its tip
agrees. Septentrio only; Trimble is reported skipped, not dropped. Exit 0
clean, 1 divergence. See [`docs/cli-surface.md`](docs/cli-surface.md).

### Site logs: one entry point

**`tostools.core.site_log.build_site_log` is the single entry point** for IGS
site log generation — both `tosGPS sitelog` and `receivers`' M3G dissemination
go through it, with `receivers` keeping only a re-export shim. The §0
previous-log finder, the filename builder and the agency resolver live in
`tostools` alongside it, because **`receivers` depends on `tostools`, never the
reverse**.

There were three renderers at one point, one documented as canonical while
being dead. Before touching the renderer import, read
[`docs/architecture/legacy-fork-unification-plan.md`](docs/architecture/legacy-fork-unification-plan.md).
...
### `tos visit` / `tos contact` — vitjun and contact writes

Vitjanir (entity-attached temporal maintenance records) and the contact
relationship/entity write stack. Full verb reference, including the
`id_maintenance` vs `id_entity` namespace split, the device-attached
vitjanir roadmap, and the "no contact-delete endpoint" constraint:
**[`docs/verbs-visit-contact.md`](docs/verbs-visit-contact.md)**.
