...
def compare_rinex_to_tos(rinex_info: Dict[str, str], 
    ...
    if "REC # / TYPE / VERS" in rinex_info:
        ...
        if receiver_info:
            ...
            if rinex_receiver:
                ...
                comparison_result["corrections"]["REC # / TYPE / VERS"] = tos_receiver.strip()
            else:
            ...
            if rinex_height:
                try:
                    ...
                    if abs(rinex_h - tos_h) > 0.001:  # 1mm tolerance
                        ...
                        comparison_result["corrections"]["ANTENNA: DELTA H/E/N"] = f"{tos_h:8.4f} 0.0000 0.0000"
                    else:
        ...
        if agency_name:
            if "OBSERVER / AGENCY" in rinex_info:
                ...
                if agency_name not in rinex_agency:
                    comparison_result["corrections"]["OBSERVER / AGENCY"] = f"GNSS Operator {agency_name}"
    
    logger.info(f"Comparison found {len(comparison_result['discrepancies'])} discrepancies")
    return comparison_result


def validate_rinex_time_range(rinex_info: Dict[str, str], 
                             tos_session: Dict[str, Any],
                             loglevel: int = logging.WARNING) -> Dict[str, Any]:
    """
...
def check_station_configuration(station_sessions: List[Dict[str, Any]], 
                               loglevel: int = logging.WARNING) -> Dict[str, List[Dict[str, Any]]]:
    """
    ...
    for session in station_sessions:
        ...
        missing_components = [comp for comp in required_components if comp not in session]
        ...
        if prev_session:
            ...
            if (prev_rx.get("model") != curr_rx.get("model") or 
                prev_rx.get("serial_number") != curr_rx.get("serial_number")):
            ...
            if (prev_ant.get("model") != curr_ant.get("model") or
                prev_ant.get("serial_number") != curr_ant.get("serial_number")):
...
def generate_qc_report(station_data: Dict[str, Any], 
    ...
    report_lines.append(f"GPS METADATA QC REPORT - {station_data.get('marker', 'Unknown').upper()}")
    report_lines.append("="*60)
    ...
    report_lines.append(f"Location: {station_data.get('lat', 0):.5f}°N, {station_data.get('lon', 0):.5f}°E")
    report_lines.append("")
    ...
    if sessions:
        ...
        end_date = max(s.get("time_to", datetime.now()) for s in sessions if s.get("time_to"))
        report_lines.append(f"Period: {start_date.strftime('%Y-%m-%d') if start_date else 'Unknown'} to {end_date.strftime('%Y-%m-%d') if end_date else 'Present'}")
    report_lines.append("")
    
    # RINEX validation summary
    if rinex_comparisons:
        total_discrepancies = sum(len(comp.get("discrepancies", {})) for comp in rinex_comparisons)
            ...
            for i, comp in enumerate(rinex_comparisons):
                if comp.get("discrepancies"):
                    ...
                    for field, diff in comp["discrepancies"].items():
                        report_lines.append(f"  {field}: RINEX='{diff.get('rinex', '')}' vs TOS='{diff.get('tos', '')}'")
