...
class Station:
    ...
    def get_session(self, session_nr: int) -> Optional[Dict[str, Any]]:
        ...
        if 0 <= session_nr < len(self.device_history):
            session = {key: value for key, value in self.data.items() if key != "device_history"}
            session["device_history"] = self.device_history[session_nr]
            return session
        return None
    
    def get_sessions_list(self, date_format: str = "%Y-%m-%d %H:%M:%S") -> List[Dict[str, Any]]:
        """
        ...
        for item in self.device_history:
            ...
            if date_format:
                ...
                session["time_from"] = time_from.strftime(date_format) if time_from else "None"
                session["time_to"] = time_to.strftime(date_format) if time_to else "None"
            else:
    ...
    def get_radome(self, date_from: datetime, date_to: datetime, 
    ...
    def get_monument_height(self, date_from: datetime, date_to: datetime,
...
def get_station_list(subsets: Optional[Dict[str, Any]] = None,
...
def generate_file_list(station: Station, 
                      ...
                      extension: str = "D.Z",
                      loglevel: int = logging.WARNING) -> List[Path]:
    """
    ...
    for session in station.device_history:
        ...
        if session_start:
            ...
            potential_file = station_path / f"{marker.lower()}{session_start.strftime('%j0.%y')}{extension}"
