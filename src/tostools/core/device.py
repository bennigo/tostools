...
def process_device_sessions(
    sessions_data: List[Dict[str, Any]], loglevel: int = logging.WARNING
) -> List[Dict[str, Any]]:
    """
...
def validate_device_consistency(
    device_history: List[Dict[str, Any]], loglevel: int = logging.WARNING
) -> Dict[str, List[str]]:
    """
    ...
    sorted_sessions = sorted(
        device_history, key=lambda x: x.get("time_from") or datetime.min
    )
