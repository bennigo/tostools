...
def site_log(station_identifier, loglevel=logging.WARNING):
    ...
    for session_nr, session in enumerate(receiver_list):
        ...
        # The subsection label occupies a FIXED 5-column field, so the colons
        # line up whatever the number's width. Writing "3.{n}  " hard-coded two
        # trailing spaces, which is right for 3.1–3.9 and pushes every 3.10+
        # header one column right of the 5-space continuation lines below it.
        # ISAK reached 3.17, so this is not hypothetical on a long-lived station.
        receiver_info += (
    ...
    for session_nr, session in enumerate(antenna_list):
        ...
        antenna_info += (
            ...
            f"\n{f'4.{session_nr + 1}':<5}Antenna Type             : {device_type}    {antenna_radome}\n"
            # ASH701073.1     SNOW
            f"     Serial Number            : {serial_number}\n"
