...
def format_station_table(stations: List[Dict[str, Any]], 
                        sortby: str = "marker",
...
def format_device_history(device_history: List[Dict[str, Any]], 
    ...
    if raw_format:
        ...
        for entry in device_history:
            ...
            for device_type in ["gnss_receiver", "antenna", "radome", "monument"]:
                if device_type in entry:
                    ...
                    values.append(list(device_data.values()) if isinstance(device_data, dict) else [str(device_data)])
        ...
        for entry in device_history:
            time_from = entry.get("time_from", "").strftime("%Y-%m-%d %H:%M:%S") if entry.get("time_from") else "None"
            time_to = entry.get("time_to", "").strftime("%Y-%m-%d %H:%M:%S") if entry.get("time_to") else "None"
            
            for device_type in ["gnss_receiver", "antenna", "radome", "monument"]:
                if device_type in entry:
                    device_data = entry[device_type]
                    details = json.dumps(device_data, default=str) if isinstance(device_data, dict) else str(device_data)
